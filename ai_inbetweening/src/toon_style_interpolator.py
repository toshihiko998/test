...
class ToonStyleInterpolator:
    ...
    def interpolate(
        ...
        Returns:
            補間フレームのリスト
        """
        frames = frame1.astype(np.float32) / 255.0
        frames2 = frame2.astype(np.float32) / 255.0
        ...
        for i in range(1, num_frames + 1):
            ...
            if preserve_edges:
                ...
                blended = self._apply_edge_preservation(
                    frame1, frame2, blended, edge_mask, t
                )
            
            # 色域補正
            if use_color_correction:
                blended = self._apply_color_correction(frame1, frame2, blended, t)
    ...
    def _detect_edges(frame1: np.ndarray, frame2: np.ndarray) -> np.ndarray:
        ...
        # 正規化
        edge1 = np.clip(edge1 / (np.max(edge1) + 1e-6), 0, 1)
        edge2 = np.clip(edge2 / (np.max(edge2) + 1e-6), 0, 1)
    ...
    def _apply_edge_preservation(
        ...
        # エッジ領域では元フレームを使用
        frame1_edge = frame1[..., :3] / 255.0 if frame1.shape[2] == 3 or frame1.shape[2] == 4 else frame1
        frame2_edge = frame2[..., :3] / 255.0 if frame2.shape[2] == 3 or frame2.shape[2] == 4 else frame2
    ...
    def _apply_color_correction(
        ...
        # RGB -> HSV
        hsv1 = rgb2hsv(frame1 / 255.0)
        hsv2 = rgb2hsv(frame2 / 255.0)
    ...
    def interpolate_with_edge_linking(
        ...
        if keypoints1 is not None and keypoints2 is not None and len(keypoints1) >= 3:
            # 特徴点ベースの補間
            ...
            # フォールバック：通常の補間
            return self.interpolate(frame1, frame2, num_frames)
